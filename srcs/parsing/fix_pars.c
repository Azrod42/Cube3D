...
/*   Updated: 2023/02/13 16:01:05 by lfantine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	tagrandmerelapute(t_system *sys, char **map_t, int m, int i)
{
	while (map_t[m])
	{
...
char	**fixing(char	*buf, t_system *sys, char **map_t, int i)
{
	...
	while (map_t[i])
		i++;
	sys->map = ft_calloc(i, sizeof(char *));
	printf("size file = %d\n", i);
	...
	tagrandmerelapute(sys, map_t, mapl, 0);
